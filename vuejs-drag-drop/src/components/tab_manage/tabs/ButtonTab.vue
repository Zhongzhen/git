<template>
  <draggable class="list-group button-tab" v-model="buttonList" :options="buttonOptions" @start="isDragging=true" @end="isDragging=false">
    <div v-for="(element, index) in buttonList" :key="index">
      ...
      <double-button v-if="element.type === 23" :element="element" :isEditable="false" :formList="formList" :index="index"></double-button>
      <!-- <single-button v-if="element.type === 23" :element="element" :isEditable="false" :formList="formList" :index="index"></single-button> -->
    </div>
  </draggable>
</template>
...
import DoubleButton from '../../field_components/buttons/DoubleButton'
export default {
  name: 'ButtonTab',
  components: {
    FileButton,
